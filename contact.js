window.addEventListener("DOMContentLoaded", () => {
  ...
  /* ============================================================
     SCRAMBLE  — moved before boot so revealMain can call cascade.
     Faster step (40 ms) = more aggressive than nav hover (75 ms).
     ============================================================ */
  const SCRAMBLE_CHARS =
    "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789#!/\\@%&$*?<>=+-—:;.~^|";
  const STEP_MS   = 40;
  const STAGGER   = 140;   // ms between each cascade element
  ...
  /* ============================================================
     CASCADE SCRAMBLE
     All [data-orig] elements start as random noise, then decode
     left-to-right in a staggered wave — reads as the printer
     "printing" the text in real time.
     ============================================================ */
  function cascadeScramble() {
    if (noMotion) return;
    const targets = [...document.querySelectorAll("[data-orig]")];

    // Width-lock inline/flex elements after fonts are loaded
    // so scramble glyphs can't reflow the layout.
    targets.forEach(el => {
    ...
    // Blast everything to chaos immediately
    targets.forEach(el => {
    ...
    // Decode each target in sequence with stagger
  ...
  // Called after boot/page-reveal — waits for fonts then fires cascade
  ...
  /* ============================================================
     BOOT SEQUENCE  — fires cascade from revealMain
     ============================================================ */
  (function initBoot() {
  ...
  /* ============================================================
     PAPER / SLOT MECHANICS  (identical to main script.js)
     ============================================================ */
  ...
  /* ============================================================
     NAV SCRAMBLE  — same as main script: lock widths, hover decode
     ============================================================ */
  (document.fonts ? document.fonts.ready : Promise.resolve()).then(() => {
  ...
  /* ============================================================
     CUSTOM CURSOR  — same amber crosshair as main site
     ============================================================ */
  (function () {
